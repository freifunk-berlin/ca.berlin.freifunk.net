...
def cert_createTar(certid, directory):
    ...
    certtar.add(join(app.config['DIRECTORY'], 'freifunk_%s.key' % certid), ('ffuplink_%s.key' % certid))
    certtar.add(join(app.config['DIRECTORY'], 'freifunk_%s.crt' % certid), ('ffuplink_%s.crt' % certid))
